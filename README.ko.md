...
코딩 에이전트가 설계 문서, 비교 분석, 리서치 리포트, 슬라이드 덱을 만들어 채팅창에 쏟아냅니다. 검토하기 어렵고 내일이면 스크롤 속으로 사라지죠. dropboard는 에이전트에게 산출물을 제대로 된 웹페이지로 게시하는 명령 하나를 주고, 당신에게는 무엇을 남길지 정하는 받은함과 다시 찾아볼 작업을 보관하는 라이브러리를 줍니다.

```
나:  "board에 올려줘"
AI:  dropboard publish out.html
     --type review --summary …
나:  검토 → 아카이브하거나 라이브러리에 보관
```

![데스크톱 받은함 — 유형 도장, 눈에 띄는 미읽음 표시, 버전 정보, 카운트다운이 있는 임시 그룹](docs/screenshots/desktop-inbox.png)

![데스크톱 라이브러리 — 프로젝트와 중첩 폴더별로 정리된 장기 자료](docs/screenshots/desktop-library.png)
...
2. **당신은 리뷰합니다.** 미읽음·검색·유형 필터가 있는 모바일 퍼스트 받은함. 문서는 폰에 맞게 흐르고, 프레젠테이션은 큰 화면이 필요하다고 미리 알린 뒤 전체화면이나 새 탭으로 열 수 있습니다.
3. **처리 기록과 지식 자산을 분리합니다.** 받은함에서 끝낸 작업은 아카이브로, 책·매뉴얼·레퍼런스처럼 관리할 자료는 프로젝트와 폴더가 있는 라이브러리로 보냅니다. 완성된 장기 자료는 처음부터 라이브러리에 게시할 수 있고, 휘발성 항목은 2시간 뒤 스스로 사라집니다.

## 이름의 유래

동작 그대로입니다: 에이전트가 산출물을 **보드(board)** 에 **툭 올려두면(drop)** 받은함이 당신이 볼 때까지 붙들고, 아카이브는 처리 기록을, 라이브러리는 의도적으로 관리할 장기 자료를 보관합니다. 도장 뱃지는 검토인지, 결정인지, 그냥 읽으면 되는지 한눈에 알려줍니다.
...
- **채팅이 아니라 검토와 참조를 위해 만들어졌습니다.** 미읽음 표시와 유형 도장이 있는 받은함에서, 완료 기록은 아카이브로, 다시 볼 자료는 프로젝트·폴더·리비전·고정 문서 ID를 갖춘 라이브러리로 이어집니다.
- **원하면 휘발됩니다.** `--temp` 항목은 스스로 만료됩니다(기본 2시간) — "그냥 HTML로 보여줘"가 받은함을 어지럽히지 않고, 남길 가치가 있는 것만 탭 한 번으로 보존합니다.
- **오픈소스이고, 셀프호스팅이라 프라이빗합니다.** 산출물이 내 머신을 떠나지 않습니다. UI는 PIN 로그인, 게시 API는 Bearer 토큰으로 보호되고, 제한적인 iframe sandbox가 AI 생성 JS를 격리합니다.
...
- **산출물의 표현력을 제한하지 않습니다.** 마크다운 메모, 반응형 HTML 문서, 책 규모의 리더, 고정 비율 프레젠테이션을 게시할 수 있습니다. 인라인 차트·토글·시뮬레이션·키보드 탐색·전체화면이 동작합니다.
...
dropboard update <문서-id> <파일> [--view document|presentation|reader]
                      [--to inbox|library]
                      [--note 변경내용] [--expected N]
...
기본값인 `--view document`는 화면에 맞춰 흐르는 리포트와 문서에 사용합니다. 고정 비율 슬라이드 덱이나 키보드 중심 아티팩트에는 `--view presentation`을 사용합니다. 장·목차·검색처럼 긴 호흡으로 읽는 책, 매뉴얼, 대용량 참고자료에는 `--view reader`를 사용합니다. 프레젠테이션은 카드에서 미리 표시되고, 작은 화면에는 1024px 이상 권장 안내가 나오며, 비공개·공개 뷰어 모두 전체화면과 새 탭 열기를 지원합니다. 기존 항목은 마이그레이션 없이 `document`로 취급됩니다.

게시 목적지는 기본값인 `--to inbox`와 `--to library` 중에서 고릅니다. 검토나 의사결정이 남아 있으면 받은함으로 보내고, 완성된 책·매뉴얼·참고자료처럼 바로 보존할 지식은 라이브러리로 보냅니다. `--temp`는 받은함 전용이므로 `--to library`와 함께 사용할 수 없습니다.
...
업데이트해도 프로젝트·폴더·태그는 유지됩니다. 기본 업데이트는 받은함으로 돌아와
미읽음이 되고 최신 수정 시각 기준으로 위에 표시되며, 완성된 참고자료를 조용히
갱신하려면 `--to library`를 명시합니다. 상세 화면의 **vN** 버튼에서 과거
...
**아카이브**는 검토가 끝난 산출물의 이력입니다. 받은함에서 아카이브하면 `/archive`에
시간순으로 쌓이며, 필요하면 받은함이나 라이브러리로 다시 옮길 수 있습니다.

**라이브러리**는 계속 찾아볼 가치가 있는 책·매뉴얼·참고자료를 직접 관리하는
공간입니다. 받은함이나 아카이브에서 **라이브러리에 보관**을 누르거나, 게시할 때
`--to library`를 사용합니다. 프로젝트와 폴더가 없는 문서는 **미분류**에 모이므로
나중에 천천히 정리할 수 있습니다.
...
각 파일에는 반응형 문서, 고정 비율 프레젠테이션, 긴 형식의 리더를 위한 별도 품질 규칙이 있습니다. 모두 self-contained이고 sandbox에서 안전해야 하며, 문서는 모바일 퍼스트, 프레젠테이션은 데스크톱·태블릿 중심, 리더는 목차와 긴 읽기 흐름에 맞게 구성합니다.
...
아이템을 열고 공유 아이콘을 누르면 24시간짜리 서명된 공개 링크(`/s/<id>?...`)가 만들어져 클립보드에 복사되고, 즉시 회수할 수 있는 옵션도 함께 뜹니다. 링크를 받은 사람은 PIN 없이 그 아이템 하나만 볼 수 있고, 받은함·아카이브·라이브러리·휴지통은 볼 수 없습니다. 회수하거나 다시 공유(링크가 새로 발급됨)하면 그 전에 발급된 링크는 아직 만료 전이라도 전부 무효화됩니다.
...
1인용으로 설계됐습니다. 접근 경로: PIN → 장기 서명 세션 쿠키(UI), Bearer 토큰(API/CLI), 단기 서명 URL(아티팩트 iframe — 샌드박스라 쿠키를 보내지 않기 때문), 공개 공유 링크(위 참고 — epoch로 회수 가능, 최대 24시간). 아티팩트는 `sandbox allow-scripts`와 제한적인 CSP로 렌더링되어 쿠키·스토리지·부모 DOM에 접근하거나 폼·외부 네트워크로 데이터를 보낼 수 없습니다. 뷰어는 전체화면 권한만 추가로 위임합니다. self-contained 산출물을 위한 인라인 CSS/JS와 `data:`/`blob:` 미디어는 계속 사용할 수 있습니다.
