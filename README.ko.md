...
3. **쌓아두지 않고 정리합니다.** 쓸모 있는 작업은 프로젝트와 중첩 폴더가 있는 라이브러리로 보내고, 나머지는 실행취소 가능한 휴지통으로 보냅니다. "그냥 보여줘" 용도는 2시간 뒤 사라지는 **휘발성**으로 게시할 수 있습니다.

## 이름의 유래

동작 그대로입니다: 에이전트가 산출물을 **보드(board)** 에 **툭 올려두면(drop)** 받은함이 당신이 볼 때까지 붙들고, 라이브러리가 나중에 다시 볼 것을 보관합니다. 도장 뱃지는 검토인지, 결정인지, 그냥 읽으면 되는지 한눈에 알려줍니다.
...
- **채팅이 아니라 검토와 참조를 위해 만들어졌습니다.** 미읽음 표시와 유형 도장이 있는 받은함이 프로젝트·폴더·리비전·고정 문서 ID를 갖춘 검색 가능한 라이브러리로 이어집니다.
...
- **산출물의 표현력을 제한하지 않습니다.** 마크다운 메모, 반응형 HTML 문서, 고정 비율 프레젠테이션을 게시할 수 있습니다. 인라인 차트·토글·시뮬레이션·키보드 탐색·전체화면이 동작합니다.
...
dropboard update <문서-id> <파일> [--view document|presentation]
...
기본값인 `--view document`는 화면에 맞춰 흐르는 반응형 콘텐츠에 사용하며 모바일 퍼스트 규칙을 적용합니다. 고정 비율 슬라이드 덱이나 키보드 중심 아티팩트에는 `--view presentation`을 사용합니다. 프레젠테이션은 카드에서 미리 표시되고, 작은 화면에는 1024px 이상 권장 안내가 나오며, 비공개·공개 뷰어 모두 전체화면과 새 탭 열기를 지원합니다. 기존 항목은 마이그레이션 없이 `document`로 취급됩니다.
...
상단 **라이브러리** 탭은 기존 `archived` 상태와 `/archive` 경로를 그대로
사용하므로 API와 북마크 호환성이 유지됩니다. 문서를 보관하면 검토 대기열에서
빠지고 라이브러리로 이동합니다. 프로젝트와
...
각 파일에는 반응형 문서와 고정 비율 프레젠테이션을 위한 별도 품질 규칙이 있습니다. 둘 다 self-contained이고 sandbox에서 안전해야 하지만, 문서는 모바일 퍼스트를 유지하고 프레젠테이션은 데스크톱·태블릿 화면을 명시적으로 대상으로 삼을 수 있습니다.
