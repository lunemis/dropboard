...
1인용으로 설계됐습니다. 접근 경로: PIN → 장기 서명 세션 쿠키(UI), Bearer 토큰(API/CLI), 단기 서명 URL(아티팩트 iframe — 샌드박스라 쿠키를 보내지 않기 때문), 공개 공유 링크(위 참고 — epoch로 회수 가능, 최대 24시간). 아티팩트는 `sandbox allow-scripts`와 CSP sandbox 헤더로 렌더링되어 쿠키·스토리지·부모 DOM에 접근할 수 없습니다.
